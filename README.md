...
**Ожидаемые логи до скоринга модели:**
```
fraud_model | INFO - Waiting for files in the input directory...
```

#### Процесс скоринга:
1. Ожидание строки из логов выше, что сервис готов принимать данные
2. Поместите CSV файл (например `test.csv` из соревнования ) в директорию `./input/` (**см. 📊 Использование сервиса**)
3. Сервис автоматически обнаружит файл
4. Выполнит предобработку и скоринг
5. Сохранит результаты в `./output/` с временной меткой
